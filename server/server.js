...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (passwordMatch) {
      var sql =
        "select balance from account where user_name='" +
        userName +
        "' and currency = '" +
        currency +
        "'";
      ...
      if (jsonObj.length == 0) {
        ...
        if (dbBal >= amount) {
          ...
          sql =
            "insert into payment (payer, payee, amount, dp, currency, payment_desc, date_time) values " +
            "('" +
            userName +
            "','" +
            payee +
          ...
          if (jsonObj.length == 0) {
            // open acct
            sql =
              "insert into account (user_name, balance, dp, currency) values " +
              "('" +
              payee +
              "'," +
              amount +
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    var sql =
      "select user_name from user_credential where user_session = '" +
      sessionid +
      "' and user_name = '" +
      userName +
      "'";
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    var jsonHash = await checkUserDB(userName);
    console.log(
      "===============password checked  " + jsonHash[0].password_hash
    );
    ...
    if (passwordMatch) {
      ...
      var sql =
        "update user_credential set user_session='" +
        hashwords +
        "' where user_name='" +
        userName +
        "'";
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    var userName = req.body.userName;
    var sql =
      "select user_name, balance, currency from account where user_name = '" +
      userName +
      "' ";
    console.log(sql);
    var jsonHash = await checkDB(sql);
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    var sql =
      "insert into deposit (user_name, amount, dp, currency, from_wallet, deposit_date) values " +
      "('" +
      userName +
      "'," +
      amount +
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    var userName = req.body.userName;
    var sql =
      "select user_name, amount, dp, currency, from_wallet, deposit_date from deposit where user_name = '" +
      userName +
      "' ";
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    var sql =
      "insert into exchange (user_name, from_amount, from_dp, from_currency, to_amount, to_dp, to_currency, exchange_date) values " +
      "('" +
      userName +
      "'," +
      fromAmount +
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    var userName = req.body.userName;
    var sql =
      "select    user_name, from_amount, from_dp, from_currency, to_amount, to_dp, to_currency," +
      "exchange_date from exchange where user_name = '" +
      userName +
      "' ";
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    var sql =
      "select user_name, balance, dp, currency from account where user_name = '" +
      userName +
      "'";
    app.post(
      "/api/getAccounts",
...
app.post(
  "/api/getPayerPayments",
  asyncHandler(async (req, res, next) => {
    var payer = req.body.payer;

    var sql =
      "select payer, payee, amount, dp, currency, payment_desc, date_time from payment where payer = '" +
      payer +
      "'";
...
app.post(
  "/api/getPayeePayments",
  asyncHandler(async (req, res, next) => {
    var payee = req.body.payee;

    var sql =
      "select payer, payee, amount, dp, currency, payment_desc, date_time from payment where payee = '" +
      payee +
      "'";
    console.log(sql);
    var jsonHash = await checkDB(sql);
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    var sql =
      "select payer, payee, amount, dp, currency, payment_desc, date_time from payment";
...
async function insertUserDB(userName, passwordHash) {
  var sql =
    "insert into user_credential (user_name, password_hash) values ('" +
    userName +
    "','" +
    passwordHash +
    "')";
...
async function checkUserDB(userName) {
  console.log("check user db");
  var sql =
    "select password_hash from user_credential where user_name = '" +
    userName +
    "' ";
  var res;
  try {
